...
func HasCSAMViolationMarker(err *types.FocusAPIError) bool {
	if err == nil {
		return false
	}
...
func WrapAsViolationFeeGrokCSAM(err *types.FocusAPIError) *types.FocusAPIError {
	if err == nil {
		return nil
	}
...
func NormalizeViolationFeeError(err *types.FocusAPIError) *types.FocusAPIError {
	if err == nil {
		return nil
	}
...
func shouldChargeViolationFee(err *types.FocusAPIError) bool {
	if err == nil {
		return false
	}
...
func ChargeViolationFeeIfNeeded(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, apiErr *types.FocusAPIError) bool {
